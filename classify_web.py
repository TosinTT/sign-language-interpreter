"""
Real-Time Sign Language Detection with TensorFlow 2+ and Voice Output
Fixed for Raspberry Pi with improved TTS and deletion functionality
...
class SignLanguageDetector:
    def __init__(self, model_path, labels_path):
        ...
        # Initialize TTS system for Raspberry Pi
        self.tts_method = self.setup_tts()
        
        # Create a lock for speech engine to prevent overlapping speech
        self.speech_lock = threading.Lock()
        
    def setup_tts(self):
        """Setup text-to-speech system for Raspberry Pi"""
        print("Setting up text-to-speech system...")
        
        # Try different TTS methods in order of preference
        tts_methods = [
            ("espeak", self.speak_with_espeak),
            ("festival", self.speak_with_festival),
            ("pyttsx3", self.speak_with_pyttsx3),
            ("system", self.speak_with_system)
        ]
        
        for method_name, method_func in tts_methods:
            if self.test_tts_method(method_name):
                print(f"Using TTS method: {method_name}")
                return method_func
        
        print("Warning: No TTS method available. Voice output disabled.")
        return None
    
    def test_tts_method(self, method):
        """Test if a TTS method is available"""
        try:
            if method == "espeak":
                ...
                return True  # System call always available as fallback
        except:
            return False
        return False
    
    def speak_with_espeak(self, text):
        """Speak using espeak (recommended for Raspberry Pi)"""
        try:
    ...
    def speak_with_system(self, text):
        """Speak using system command (last resort)"""
        try:
            # Try aplay with espeak pipe
            process = subprocess.Popen(["espeak", "-s", "150", "--stdout"], 
                                     stdin=subprocess.PIPE, 
                                     stdout=subprocess.PIPE, 
                                     stderr=subprocess.PIPE)
    ...
    def speak_text(self, text):
        """Speak the given text using the available TTS method"""
        if not text or not self.tts_method:
            return
